...
class CriticalPointsPlotter(GridPlotter):
    ...
    def _create_ring_polyline(self, coords, n_sides=12):
        coords = self._smooth_ring_polar(coords, window=4, n_iter=3)

        coords = self._resample_closed_curve(coords, 20)

        n = len(coords)
        points = np.vstack([coords, coords[0]])

        lines = (
            np.column_stack((np.full(n, 2), np.arange(n), np.arange(1, n + 1)))
            .astype(np.int64)
            .ravel()
        )
        ...
        return poly.tube(
            radius=self.critical_radii * 0.1, n_sides=n_sides, capping=True
        )
    ...
    def _create_all_crit_meshes(self):
        ...
        for i, name in enumerate(("minima", "saddle1", "saddle2", "maxima")):
            point_polydata, ring_meshes, cage_meshes = self._create_crit_meshes_by_type(
                i
            )
        ...
        for i, name in enumerate(("minima", "saddle1", "saddle2", "maxima")):
            ...
            if point_polydata:
                ...
                glyphs = point_polydata.glyph(
                    geom=self._sphere_mesh, factor=radii, orient=False
                )
                # add to plot
