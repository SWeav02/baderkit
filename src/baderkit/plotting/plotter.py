...
class StructurePlotter:
    ...
    def radii(self, radii: list[float]):
        ...
        # then remake
        for i, (site, old_r, new_r, color) in enumerate(zip(self.structure, old_radii, radii, self.colors)):
            if old_r == new_r:
                continue
    ...
    def camera_position(self):
        ...
        return [
            list(pos[0]), list(pos[1]), list(pos[2])
            ]
    ...
    def camera_position(self, camera_position: NDArray):
        ...
        if camera_position.ndim == 1:
            ...
            # convert to tuples
            camera_position = [
                tuple(camera_position[0]), tuple(camera_position[1]), tuple(camera_position[2])
                ]
