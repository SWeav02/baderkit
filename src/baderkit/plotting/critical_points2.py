...
from baderkit.critical_points import CriticalPoints

from .defaults import CONNECTION_COLORS, CRIT_COLORS
...
class CriticalPointsPlotter(GridPlotter):
    def __init__(
        ...
        **grid_kwargs,
        ...
        self._critical_point_colors = np.array(
            [
                pv.Color(CRIT_COLORS.get(crit_type, "#FFFFFF")).float_rgb
                for crit_type in range(4)
            ]
        )
        ...
        self._connection_colors = np.array(
            [
                pv.Color(CONNECTION_COLORS.get(crit_type, "#FFFFFF")).float_rgb
                for crit_type in range(3)
            ]
        )
    ...
    def critical_point_colors(self, colors: list[str]):
        ...
        crit_colors = np.array(
            [pv.Color(i).float_rgb for i in colors], dtype=np.float64
        )
    ...
    def critical_point_radii(self, critical_point_radii: NDArray[float]):
        ...
        assert (
            self.critical_point_radii.shape == critical_point_radii.shape
        ), "Length match the number of critical points"
    ...
    def _split_and_wrap_line_frac(p0, p1, eps=1e-12):
        ...
        for i in range(3):
            if abs(v[i]) < eps:
                continue
            for m in range(
                int(np.floor(min(p0[i], p1[i]))), int(np.ceil(max(p0[i], p1[i])))
            ):
                t = (m - p0[i]) / v[i]
                if eps < t < 1 - eps:
                    ts.append(t)
    ...
    def _get_connection_mesh(self, p0, p1, tol=0.01) -> pv.PolyData:
        ...
        for f0, f1 in segments:
            ...
            # add line segment
            unwrapped_line_actors.append(pv.Line(c0, c1))
            wrapped_line_actors.append(pv.Line(c0, c1))
    ...
    def _update_connection_meshes(self):
        ...
        for idx, (color, visible) in enumerate(
            zip(connection_colors, visible_connections)
        ):
            if not visible:
