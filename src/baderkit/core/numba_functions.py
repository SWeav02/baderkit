...
def get_best_neighbor(
    ...
    base = data[i, j, k]
    best = 0.0
    best_shift = np.zeros(3, dtype=np.int64)
    ...
    for i in prange(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                best_shift, best_neigh = get_best_neighbor(
                    data=data,
                    i=i,
                    j=j,
                    k=k,
                ...
                best_label[i,j,k] = initial_labels[best_neigh[0],best_neigh[1],best_neigh[2]]
    return best_label
