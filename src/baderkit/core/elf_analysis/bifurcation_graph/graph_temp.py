...
class BifurcationGraph(BaseAnalysis):
    ...
    def __init__(
        ...
        calculate_surrounded_atoms: bool = False,
        **kwargs,
    ):
    ...
    def bifurcations(self) -> dict:
        if self._bifurcations is None:
            ...
            bif_mask = find_potential_saddle_points(
                ...
                greater=True,
                vacuum_mask=reference_grid.vacuum_mask,
            )
            ...
            basin_maxima_grid = np.round(
                reference_grid.frac_to_grid(bader.basin_maxima_frac)
            ).astype(np.int64)
...
###############################################################################
    ...
    @classmethod
    def from_bader(cls, charge_bader: Bader, bader: Bader, **kwargs):
        """
