...
def get_manifold_labels_doublegrid(
    ...
    for group in minima_groups:
        for i,j,k in group:
            ...
            j*=2
            k*=2
            # iterate over all neighboring faces/edges/vertices
            for si,sj,sk in INT_TO_IMAGE:
                ni, nj, nk = wrap_point(i+si, j+sj, k+sk, nx2, ny2, nz2)
                edges[ni,nj,nk] = 0
...
def get_canonical_saddle_connections_doublegrid(
    ...
    canonical_saddle_connections = np.empty((len(saddle_coords),3),dtype=np.uint16)
    extrema_connections = np.empty((len(saddle_coords),4),dtype=np.uint16)
