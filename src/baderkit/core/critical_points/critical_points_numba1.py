    ...
    for parity, dim in zip(parities, parity_dims):
        ...
        for facet_transform in facet_transforms:
            ...
            for _ in range(dim - 1):
                new_transforms = []
                new_parities = []
                for current_trans, current_par in zip(
                    current_transforms, current_parities
                ):
        ...
        # get unique vertices
        facet_vertices, indices, inverse = np.unique(
            facet_vertices, axis=0, return_index=True, return_inverse=True
        )
        # reduce indices
...
def get_cofacet_vertices():
    ...
    for parity, dim in zip(parities, parity_dims):
        ...
        current_parity_vertices, indices, inverse = np.unique(
            current_parity_vertices, axis=0, return_index=True, return_inverse=True
        )
        ...
        current_parity_cofacet_indices = [
            inverse[idx] for idx in current_parity_cofacet_indices
        ]
            ...
            for k in range(fk):
                ...
                if len(best_facets) > 1:
                    ...
                    for facet_idx in best_facets[1:]:
                        sorted_values = np.flip(
                            np.sort(values[facet_indices[facet_idx]])
                        )
...
def get_cell_pairs_alg2(
    ...
    # 5) maps form parity index to cofacet transformations. Does not map to
    # vertices
    cofacet_transforms, cofacet_vertices, cofacet_facets, cofacet_indices = (
        get_cofacet_vertices()
    )
    ...
    for i in range(fi):
        for j in range(fj):
            for k in range(fk):
                ...
                for trans_idx, ((si, sj, sk), facet_indices) in enumerate(
                    zip(cofacet_trans, cofacet_fac)
                ):
