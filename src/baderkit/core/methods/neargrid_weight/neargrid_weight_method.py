...
from .neargrid_weight_numba import get_interior_basin_charges_and_volumes, get_edge_charges_volumes
...
class NeargridWeightMethod(MethodBase):
    ...
    def run(self):
        ...
        charges, volumes, vacuum_charge, vacuum_volume = get_interior_basin_charges_and_volumes(
        ...
        # sort edges from highest to lowest
        ...
        # create a map from edge coords to corresponding flat index in the sorted
        # list
        index_map = np.full(grid.shape, -1, dtype=np.int64)
        index_map[edge_indices[:,0],edge_indices[:,1],edge_indices[:,2]]=np.arange(len(edge_indices))
        
        # assign weighted edge charge/volume
        # breakpoint()
        charges, volumes = get_edge_charges_volumes(
            ...
            volumes=volumes,
            neighbor_transforms=neighbor_transforms,
            neighbor_weights=1/neighbor_dists,
            )
