...
In addition to improved speed, BaderKit fixes an issue in the original codes handling of local maxima. In highly symmetrical systems, it is common for a local maximum to sit precisely between two or more grid points. This results in multiple adjacent points with values greater than or equal to their neighbors. In the original Fortran code, these points are incorrectly considered separate maxima. `BaderKit` explicitly checks for this and combines adjacent maxima (\autoref{fig:fig2}). This is a fast process performed prior to the main calculation that results in more physically reasonable basins and faster times due to the reduction of edge points.

![Comparison of the basins found for a single Ag atom in the primitive cell of P6₃/mmc Ag (a=b=2.911Å, c=9.463Å) with a real-space grid shape of 20x20x60. The Henkelman code finds two basins at the Ag atom's maximum, while BaderKit merges the voxelated basins into a single maximum. All other charge density is masked out for visibility. The charge density is calculated by DFT using the PBE functional[@Perdew:1996], an energy cutoff of 354.4 eV, a 5x5x2 Monkhorst–Pack k-point mesh, and VASP's default GW pseudo-potentials \label{fig:fig2}](basin_reduction.png){ width=50% }
